...
model = torch.hub.load("ultralytics/yolov5", "custom", path = 'best_mushroom.pt', force_reload=True)
...
def uploaded_file(filename):
    ...
    if len(results.pandas().xyxy) > 0:
        ...
        return render_template('results_mushroom.html', confidences=format_confidences, labels=labels,
                               old_filename=filename,
                               filename=filename)
    else:
        found = False
        return render_template('results_mushroom.html', labels='No Emotion', old_filename=filename, filename=filename)
...
def results(filename):
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return redirect(url_for('uploaded_file',
                                 filename=filename))
    return render_template('results_mushroom.html')
